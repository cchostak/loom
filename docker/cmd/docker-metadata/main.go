...
func main() {
	path := "/relay/docker.sock"
	_ = os.Remove(path)
	listener, err := net.Listen("unix", path)
	if err != nil {
		log.Fatal("Metadata socket unavailable")
	}
	defer listener.Close()
	if os.Chmod(path, 0600) != nil {
		log.Fatal("Socket permissions unavailable")
	}
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if (r.Method != "GET" && r.Method != "HEAD") || r.URL.RawQuery != "" || r.URL.RawPath != "" || (!inspectPath.MatchString(r.URL.Path) && !versionPath.MatchString(r.URL.Path)) {
			http.Error(w, "denied", 403)
			return
		}
		req, _ := http.NewRequestWithContext(r.Context(), r.Method, "http://docker"+r.URL.Path, nil)
		...
		if err != nil || len(b) > 1<<20 || res.StatusCode != 200 {
			http.Error(w, "metadata unavailable", 503)
			return
		}
		if inspectPath.MatchString(r.URL.Path) {
			...
			if json.Unmarshal(b, &data) != nil {
				http.Error(w, "invalid metadata", 503)
				return
			}
			...
			if !ok || labels["com.docker.compose.project"] != os.Getenv("LOOM_COMPOSE_PROJECT") {
				http.Error(w, "denied", 403)
				return
			}
		...
		for k, v := range res.Header {
			if strings.EqualFold(k, "Api-Version") || strings.EqualFold(k, "Docker-Experimental") {
				w.Header()[k] = v
			}
		}
